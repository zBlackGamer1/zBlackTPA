...
public class TpacceptCMD implements CommandExecutor {
	...
	public boolean onCommand(CommandSender s, Command c, String arg2, String[] a) {
		...
		Player p = (Player)s;
		if(a.length != 1) {
			p.sendMessage(Main.cache.mensagensCFG.get("tpaccept-incorreto"));
			return true;
		}
		...
		p.sendMessage(Main.cache.mensagensCFG.get("tpaccept-sucesso").replace("%player%", target.getName()));
		target.sendMessage(Main.cache.mensagensCFG.get("tpaccept-sucesso1").replace("%player%", p.getName()));
		return true;
	}
}
